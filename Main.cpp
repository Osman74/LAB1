...
int main() {
	...
	std::vector<SurfaceFiniteElement> SFEs = A.getSurfaceFiniteElements();
	file << "Список поверхностных КЭ" << std::endl;
	std::for_each(SFEs.begin(), SFEs.end(), [&](SurfaceFiniteElement& A) {file << A << std::endl; });
	file << std::endl;
	...
	std::vector<SurfaceFiniteElement> SFEsbyIDSurface = A.getSFEs_byIDSurface(4);
	file << "Список поверхностных КЭ с ID поверхности = 4" << std::endl;
	std::for_each(SFEsbyIDSurface.begin(), SFEsbyIDSurface.end(), [&](SurfaceFiniteElement& A) {file << A << std::endl; });
	file << std::endl;
