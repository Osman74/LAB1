...
std::vector<SurfaceFiniteElement> cls_MeshLoader::getSurfaceFiniteElements()const {
    return m_SFEs;
}
...
std::vector<FiniteElement> cls_MeshLoader::findFE_byNodes(unsigned p_ID1, unsigned p_ID2, unsigned p_ID3)const {
	...
	while(IT != m_FEs.end())
	{
		IT = std::find_if(IT, m_FEs.end(), [&](const FiniteElement& FE) -> bool {
			bool flag1 = false;
			bool flag2 = false;
			bool flag3 = false;
...
std::vector<Node> cls_MeshLoader::getSFNs_byIDSurface(int p_IDSurface) const {
	...
	while (IT != m_SFEs.end())
	{
		IT = std::find_if(IT, m_SFEs.end(), [&](const SurfaceFiniteElement& SFE) -> bool {
									return SFE.m_IDSurface == p_IDSurface;});
		if (IT != m_SFEs.end()){
...
std::vector<FiniteElement> cls_MeshLoader::getFEs_byTypeID(int p_IDType) const {
	...
	auto IT = m_FEs.begin();
	while (IT != m_FEs.end()) {
		std::find_if(IT, m_FEs.end(), [&](const FiniteElement& FE){ return (FE.m_IDType == p_IDType);});
		if (IT != m_FEs.end()){
			p_FEs.push_back(*IT);
			++IT;
		}
...
std::vector<SurfaceFiniteElement> cls_MeshLoader::getSFEs_byIDSurface(int p_IDSurface) const {
	std::list<SurfaceFiniteElement> p_SFEs;
	auto IT = m_SFEs.begin();
	while (IT != m_SFEs.end()) {
		std::find_if(IT, m_SFEs.end(), [&](const SurfaceFiniteElement& SFE) -> bool { return (SFE.m_IDSurface == p_IDSurface);});
		if (IT != m_SFEs.end()){
			p_SFEs.push_back(*IT);
			++IT;
		}
	}
	return std::vector<SurfaceFiniteElement>(p_SFEs.begin(), p_SFEs.end());
}
...
void cls_MeshLoader::Transform_to_Quadratic() {
	...
	for (auto& IT : m_FEs) {
		...
		for (int i = 0; i < N; ++i)
			for (int j = i+1; j < N; ++j) {
				Edge current_edge(IT.m_NodeIDs[i], IT.m_NodeIDs[j], 0);
				...
				if (FoundedEdge == Edges.end()) {
					...
					Node LeftNode;
					LeftNode = m_Nodes[IT.m_NodeIDs[i]];
					Node RightNode = m_Nodes[IT.m_NodeIDs[j]];
					unsigned Center_ID = m_Nodes.rbegin()->m_ID + 1;
					Node CenterNode(Center_ID, (LeftNode.m_X1 + RightNode.m_X1) / 2,
						(LeftNode.m_X2 + RightNode.m_X2) / 2, (LeftNode.m_X3 + RightNode.m_X3) / 2, false);
